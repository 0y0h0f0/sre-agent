...
> **实现状态（已落地）**：Provider 抽象层已实现，`_build_deps()` 不再硬编码 `FakeLLM()`。
...
> - 测试：`tests/unit/test_llm_providers.py`（28 例，全部离线，网络 adapter 用 `httpx.MockTransport`）
>
> 待真实环境验证（本地 sandbox 无 GPU/网络无法跑）：`vllm` + Qwen2.5-7B-AWQ 端到端 smoke、云端 API 单次成本 < $0.05、深度推理实测延迟。这些属于 1.2 推理分层与运行时验收，不阻塞 1.1 的 adapter 落地。
...
- `llm_provider=vllm` + Qwen2.5-7B-AWQ 下，4 类 smoke eval 能完成端到端诊断；Qwen3-8B-AWQ 仅作为可选显存验证。
...
> **实现状态（已落地）**：每节点推理深度由配置驱动，可整体关闭以加速本地迭代。
...
> - 测试：`tests/unit/test_reasoning_layering.py`（14 例）
>
> 待真实环境验证：深度推理节点的实测延迟（本地 GPU smoke / 云端 API ≤10s）需真实模型，本 sandbox 无法测量。
...
> - 测试：`tests/unit/test_topology.py`（18 例）
>
