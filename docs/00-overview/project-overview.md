...
| 执行 | 默认 fixture executor；`EXECUTOR_BACKEND=live` 仅允许受限 Kubernetes restart/pause/scale/rollback |
...
- Live executor 只允许 Deployment rolling restart、Deployment rollout pause、Deployment scale/scale back、Deployment rollback subresource。
