...
- **M9 受控增强默认关闭**：LLM runbook 草稿、incident diff、web 搜索安全、Tempo trace 后端、Grafana-shaped 告警归一化与未暴露 helper、语义 runbook 搜索和外部 embedding provider 均按当前代码边界记录。生产环境默认关闭。
- **文档源关系**：`docs/` 描述当前行为；`plans/` 和 `plans/11-roadmap/` 是历史背景和 roadmap，不覆盖当前代码和安全边界。

## 当前仓库快照

以下统计来自 2026-06-17 的当前仓库结构、路由装饰器、SQLAlchemy 模型和 Docker Compose 配置。

| 维度 | 当前值 | 说明 |
|------|--------|------|
| API | 14 个 HTTP router，79 个业务 HTTP route，1 个 WebSocket | 详细契约见 [API 参考](../01-backend/api-reference.md) |
| 数据模型 | 32 个 SQLAlchemy 模型，17 个 Alembic 迁移 | 当前 embedding 字段为 512 维 |
| Agent 图 | 18 个 LangGraph 节点 | 含 collect-gap 与 verify/replan 两个有界循环 |
| 代码布局 | `apps/` 约 62 个 Python 文件，`packages/` 约 165 个 Python 文件 | 不计 `__pycache__`、构建产物和依赖目录 |
| 测试布局 | `tests/` 111 个 `test_*.py` 源文件（113 个 Python 文件含共享 helper） | 另有前端 Vitest 和 Playwright 配置 |
...
| 能力 | 当前行为 |
|------|----------|
| 告警摄取 | `POST /api/alerts` webhook、Grafana-shaped payload 归一化与 Alertmanager 轮询；按 fingerprint 去重开放 incident |
...
| 执行 | 默认 fixture executor；`EXECUTOR_BACKEND=live` 仅允许受限 Kubernetes restart/pause/resume/scale/rollback |
...
| 工程指标 | `/api/evals/engineering-metrics` 从业务库和最新 smoke eval 聚合只读工程指标 |
| M9 | AI/Web/Tempo/Grafana parser/helper/semantic search 能力均在 `M9_EXTENSIONS_ENABLED` 和子开关后面；当前没有独立 Grafana webhook router |
...
```text
Alert Webhook / Grafana-shaped Payload / Alertmanager Poll
...
- Live executor 只允许 Deployment/StatefulSet rolling restart、Deployment rollout pause/resume、Deployment scale/scale back、Deployment rollback subresource。
