...
当前 FastAPI app 暴露 83 条 HTTP route，其中 4 条为 FastAPI 自动文档/OpenAPI route：`/docs`、`/docs/oauth2-redirect`、`/redoc`、`/openapi.json`。本文统计业务 API 为 79 条 HTTP route，另有 1 条 WebSocket route。
...
- 业务错误、FastAPI `HTTPException` 和 Pydantic validation error 当前都会被包装为标准错误信封，详见 [错误响应与请求 ID](errors-and-request-ids.md)。
- API 绝不在请求线程中运行 LangGraph 诊断图；诊断通过 Celery 入队。

需要沿代码路径理解 router/service/repository、认证 scope、rate limit、事务提交点、Celery 入队和审计时，见 [API 控制面与服务层技术深挖](../00-overview/api-control-plane-service-deep-dive.md)。
需要沿代码路径理解 `/api/alerts` 的 provider payload 归一化、Alertmanager poll、Grafana-shaped payload、fingerprint、poll cursor 和 resolved inference 时，见 [Alertmanager Poll、Grafana 与告警来源归一化技术深挖](../00-overview/alert-source-normalization-poll-grafana-deep-dive.md)。
需要沿代码路径理解 NFA、root cause/action feedback、相关事件、runbook amendment review 和 memory/eval 回流边界时，见 [反馈、NFA、关联事件与持续学习技术深挖](../00-overview/feedback-nfa-correlation-continuous-learning-deep-dive.md)。
...
| Evals | 6 | `/api/evals` |
| WebSocket ticket | 1 | `/api/ws/incidents/{incident_id}/ticket` |
| WebSocket | 1 WS | `/api/ws/incidents/{incident_id}` |
...
`/api/alerts` 是当前公开的 HTTP 告警入口。它能归一化 Grafana-shaped payload；当前没有在 `apps/api/main.py` 注册独立 Grafana webhook router。Alertmanager poll 由 Celery Beat task 触发，不是 HTTP route；它最终也调用 `AlertService.create_alert()`，所以仍共享 fingerprint 去重、NFA suppression 和 Celery 入队边界。
...
| GET | `/api/incidents/{incident_id}` | incident 详情，包含根因、证据、建议动作；证据项包含 `source_id`、`source_path` 和 `metadata`，runbook 证据用它们追踪 `chunk_id` / Markdown 来源。 |
...
Report 字段包括 `report_id`、`incident_id`、`agent_run_id`、`version`、`root_cause`、`impact`、`timeline`、`actions`、`follow_ups`、`evidence_ids`、`body_markdown`、`created_at`。使用 runbook context 的报告会把对应 runbook evidence ID 纳入 `evidence_ids`。

报告生成、再生成、latest-only API、版本追加、通知和 incident/run lifecycle 的完整代码路径见 [报告生成、版本与事件生命周期技术深挖](../00-overview/report-generation-incident-lifecycle-deep-dive.md)。
...
Discovery read API、manual rerun、capability matrix、topology 和 proposal 边界见 [Discovery、Capability Matrix 与服务拓扑技术深挖](../00-overview/discovery-capability-topology-deep-dive.md)。
...
Runbook draft、version、M9 incident diff amendment 的审核和发布边界见 [Runbook 草稿、版本与 Amendment 生命周期技术深挖](../00-overview/runbook-draft-version-amendment-lifecycle-deep-dive.md)。
...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | `/api/evals/engineering-metrics` | 返回项目工程评估指标。支持 `window_days`，默认 30，范围 1-365。 |
| GET | `/api/evals/runs` | 列出 eval runs。 |
| POST | `/api/evals/runs` | 创建 eval run 并入队。HTTP 201。 |
| GET | `/api/evals/runs/{eval_run_id}` | eval run 详情。 |
| POST | `/api/evals/replay` | 创建历史 replay eval run 并入队。HTTP 201。 |
| POST | `/api/evals/shadow` | 为 incident 触发 shadow eval。HTTP 201。 |

CI smoke eval 必须使用 FakeLLM。真实 LLM 只允许 manual full eval 或手动 demo。

Replay eval 会读取历史 incident/latest run，但每个 case 在临时内存 DB 中运行 Agent，强制 fixture executor，并且只把汇总 metrics 写回 `eval_runs`；不会写真实 `actions`、`approvals` 或 `incident_reports`。

`engineering-metrics` 是只读聚合接口。它从最新成功 smoke eval 和当前业务库中的 `agent_runs`、`tool_calls`、`actions`、`approvals`、`evidence_items`、`incident_reports` 聚合质量、安全、可靠性、性能和缓存效率指标。响应包含每项指标的 `score`、`status`、`target`、`source`、`reproduction`，以及整体 `scorecard`、分类分、完整度和 top risks。覆盖率、CI 状态、API 延迟和 DORA 指标不在业务库内，响应中以 `status=unknown` 暴露，并通过 `source` 指明应由 CI、Prometheus 或 VCS/CD 系统提供。

## WebSocket

| 类型 | 路径 | 描述 |
|------|------|------|
| HTTP POST | `/api/ws/incidents/{incident_id}/ticket` | 启用 API key auth 时签发短期 WebSocket ticket。 |
