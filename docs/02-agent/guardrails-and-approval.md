...
如果需要按代码路径追踪批量审批、email token、stale auto-approve、snapshot、live executor preflight 和排障入口，见 [护栏与审批技术深挖](../00-overview/guardrail-approval-deep-dive.md)。如果需要聚焦 fixture/live executor、action capability metadata、snapshot contract、verify gates、degraded rollback 和 API fixture 直执边界，见 [执行器、动作能力与验证闭环技术深挖](../00-overview/executor-action-verification-loop-deep-dive.md)。如果需要聚焦 approval email token 的邮件、确认页、浏览器通知和操作员协作链路，见 [通知、邮件、评论协作与操作员交互技术深挖](../00-overview/notifications-collaboration-operator-interaction-deep-dive.md)。
...
| L2 | 服务/Kubernetes/资源限额运维动作 | 是 | 是 | `restart_pod`、`restart_deployment`、`scale_deployment`、`restart_service`、`restart_statefulset`、`pause_rollout`、`resume_rollout`、`increase_memory_limit`、`scale_back`、`revert_config` |
...
- 对 K8s 动作，按每个 action `target` 读取 `get_deployment` 或 `get_statefulset` 的 replicas、revision/image、ready/available/updated replicas 等信息

`take_snapshot`、`execute_action` 和 post-action `verify` 使用同一个有效 namespace：优先 `EXECUTOR_K8S_NAMESPACE`，为空时回退 `K8S_NAMESPACE`，再回退 `default`。`execute_action` 在调用 live executor 前会校验 snapshot 中的 Deployment `name` 与 action `target` 一致，并校验 snapshot namespace 与有效 executor namespace 一致。缺少 snapshot 字段或 snapshot 与目标不匹配时，live backend 不会被调用。
...
完整执行闭环还包括 action capability metadata、snapshot contract、live params whitelist、verify gates、degraded rollback 和手动 action execute API 的 fixture-only 边界；这些细节集中维护在 [执行器、动作能力与验证闭环技术深挖](../00-overview/executor-action-verification-loop-deep-dive.md)。
...
| `restart_deployment` | 同 `restart_pod`，显式表示目标是 Deployment |
...
| `resume_rollout` | patch Deployment `spec.paused=false`，恢复 rollout |
...
| bounded irreversible | `restart_pod`、`restart_deployment`、`restart_service`、`restart_statefulset`、`pause_rollout`、`resume_rollout` | 只允许受控 Deployment/StatefulSet patch；需要 snapshot、preflight 和 verify gates，但不提供 restore/undo 保证 |

`restart_pod` / `restart_deployment` / `restart_service` 不应被文档、prompt 或 report 描述为“可完全恢复”或“可回滚”的动作。它们只是受审批、snapshot、preflight、verify/replan 和审计约束的 bounded irreversible 操作；如果验证降级，下一轮 planner 只能基于 snapshot 规划 `scale_back`、`rollback_release`、`rollback_deployment`、`revert_config` 等可执行回退/升级动作，不能假设 restart 本身有 undo。

`restart_statefulset` 也是 L2 bounded irreversible 动作，只允许 patch StatefulSet pod template annotation。它要求执行前 snapshot 中 `kind=StatefulSet`、replicas、ready replicas 和 image 均存在；不得用于任意 StatefulSet patch、scale 或 PVC/storage 变更。

`pause_rollout` 和 `resume_rollout` 都是 L2 动作，只能通过 Deployment patch 切换 `spec.paused`。`pause_rollout` 要求执行前 snapshot 中 `paused=false`；`resume_rollout` 要求 `paused=true`。二者都不等同于 rollback，仍必须经过 approval、snapshot、preflight、verify/replan 和审计。

`scale_deployment` 只表示调整 Deployment 副本数，参数只能使用 `replicas`。live preflight 和 executor 共用同一套校验：`replicas` 必须是整数，范围为 0 到 50；缺失、非整数、负数、超过上限或混入 `memory_limit` 等非副本参数时，都会在调用 Kubernetes API 前阻断。`scale_back` 在降级回退路径可从 pre-action snapshot 读取原始 replicas。内存限额调整使用 `increase_memory_limit`，当前仅在 fixture/local 路径有确定性执行结果，不属于 live executor 的真实 Kubernetes mutation。

`rollback_release` / `rollback_deployment` 只能调用 Deployment rollback subresource。live preflight 要求目标 Deployment snapshot 存在，并且 action 参数只能包含可选的 `to_revision`。未传 `to_revision` 时，普通 rollback 使用 Kubernetes rollback subresource 的默认上一版语义；降级回退路径可以从 pre-action snapshot 读取原始 revision。显式 `to_revision` 或从 snapshot 读取的 revision 都必须是正整数，否则在调用 Kubernetes API 前阻断。
...
| `k8s_rollout` | `K8sDiagnosticsTool.rollout_status` / `get_statefulset` | 只读 K8s diagnostics，不执行 rollout 写入 |
