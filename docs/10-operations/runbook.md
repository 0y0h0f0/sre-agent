...
| `agentp_llm_prompt_tokens_total` / `agentp_llm_completion_tokens_total` | LLM prompt/completion token 统计，label: `model`, `provider` |
| `agentp_llm_cached_prompt_tokens_total` | Provider 明确报告的 cached prompt token 统计，label: `model`, `provider` |
| `agentp_llm_provider_cache_status_total` | Provider prompt cache 三态观测，label: `model`, `provider`, `status` (`hit`/`miss`/`unknown`) |
| `agentp_llm_cache_hit_total` / `agentp_llm_cache_miss_total` | Provider cache legacy hit/miss 计数；`unknown` 不进入 miss |
| `agentp_llm_call_duration_seconds` | LLM 调用耗时直方图，label: `model`, `provider` |
| `agentp_llm_call_errors_total` | LLM 调用错误 |
| `agentp_llm_json_repair_attempts_total` | JSON repair 尝试次数，label: `node` |
| `agentp_llm_fallback_total` | LLM fallback 次数，label: `node`, `reason`，reason 为固定枚举 |
| `agentp_web_search_requests_total` / `agentp_web_search_duration_seconds` | Web search 尝试和耗时，label 使用固定 provider/status/reason |
| `agentp_web_search_results_total` | URL safety 过滤后的安全结果数 |
| `agentp_web_search_blocked_total` | Web search 被策略/URL safety 阻断的次数，reason 为固定枚举 |
| `agentp_web_search_query_redactions_total` | Web search query 出 provider 前的脱敏计数 |
| `agentp_web_search_cache_status_total` | Web context cache 状态，status 为 `hit`、`miss`、`unknown` 或 `not_applicable` |
...
LLM/Web 指标 label 必须保持低基数。不要把 prompt、completion、异常文本、query、URL、URL path、host、customer ID 或 secret 放入 label；Web URL 只能作为已验证结果 traceability 字段保存。
