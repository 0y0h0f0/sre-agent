...
def _diag(
    ...
    return {
        "hypotheses": [
            {"id": "h1", "statement": h1[0], "supporting_evidence_ids": [],
             "confidence": h1[1], "rank_explanation": h1[2]},
            {"id": "h2", "statement": h2[0], "supporting_evidence_ids": [],
             "confidence": h2[1], "rank_explanation": h2[2]},
        ],
...
def _action(
    ...
    reason: str, risk_hint: str, rollback_plan: str = "",
) -> dict[str, Any]:
    return {
        "type": action_type, "target": target, "params": params,
        "reason": reason, "risk_hint": risk_hint, "rollback_plan": rollback_plan,
    }
...
_DIAGNOSIS_MAP: dict[str, dict[str, Any]] = {
    "DatabaseConnectionExhaustion": {
        "hypotheses": [
            {"id": "h1", "statement": "Connection pool saturated by slow queries",
             "supporting_evidence_ids": [], "confidence": 0.88,
             "rank_explanation": "Metrics show elevated active connections"},
            {"id": "h2", "statement": "Idle-in-transaction connections blocking pool",
             "supporting_evidence_ids": [], "confidence": 0.55,
             "rank_explanation": "DB diagnostics show idle-in-transaction state"},
        ],
        "root_cause": {"summary": "DB connection pool exhausted by slow query accumulation",
                       "confidence": 0.88, "evidence_ids": []},
        "evidence_ids": [],
        "missing_evidence": ["slow query logs", "connection pool config"],
    },
    "High5xxAfterDeploy": {
        "hypotheses": [
            {"id": "h1", "statement": "Recent deployment introduced a code regression",
             "supporting_evidence_ids": [], "confidence": 0.83,
             "rank_explanation": "Deployment and 5xx spike are temporally correlated"},
            {"id": "h2", "statement": "Downstream dependency degraded after deploy",
             "supporting_evidence_ids": [], "confidence": 0.55,
             "rank_explanation": "Downstream error rates also elevated"},
        ],
        "root_cause": {"summary": "Recent deployment introduced a code regression",
                       "confidence": 0.83, "evidence_ids": []},
        "evidence_ids": [],
        "missing_evidence": ["deployment diff", "error stack traces"],
    },
    "RedisCacheAvalanche": {
        "hypotheses": [
            {"id": "h1", "statement": "Cache miss storm due to key eviction or TTL expiry",
             "supporting_evidence_ids": [], "confidence": 0.80,
             "rank_explanation": "Cache hit rate collapsed; DB QPS surged"},
            {"id": "h2", "statement": "Redis instance degraded or network partition",
             "supporting_evidence_ids": [], "confidence": 0.60,
             "rank_explanation": "Redis connection errors in logs"},
        ],
        "root_cause": {"summary": "Cache avalanche from mass key expiry",
                       "confidence": 0.80, "evidence_ids": []},
        "evidence_ids": [],
        "missing_evidence": ["TTL distribution", "hot key report"],
    },
    "PodRestartLoop": {
        "hypotheses": [
            {"id": "h1", "statement": "Memory leak causing OOMKilled and restart",
             "supporting_evidence_ids": [], "confidence": 0.88,
             "rank_explanation": "Memory metrics trending to limit"},
            {"id": "h2", "statement": "Startup probe timeout due to slow initialization",
             "supporting_evidence_ids": [], "confidence": 0.45,
             "rank_explanation": "Startup probe failing after deployment"},
        ],
        "root_cause": {"summary": "Memory leak causing OOMKilled; pod restart loop",
                       "confidence": 0.88, "evidence_ids": []},
        "evidence_ids": [],
        "missing_evidence": ["heap profile", "memory limit config"],
    },
    "CPUThrottling": _diag(
        "CPU throttling — resource limits too low for workload spike", 0.82,
    ...
    "MemoryLeak": _diag(
        "Memory leak in application — heap growing without bound", 0.85,
    ...
    "DiskFull": _diag(
        "Disk space exhausted on node or PVC", 0.90,
    ...
    "CertificateExpiry": _diag(
        "TLS certificate expiring — connections failing validation", 0.92,
    ...
    "DNSFailure": _diag(
        "DNS resolution failures — upstream names unreachable", 0.78,
    ...
    "MessageQueueLag": _diag(
        "Message queue consumer lag — backlog accumulating", 0.84,
    ...
    "RateLimitTriggered": _diag(
        "Rate limit triggered — requests being throttled", 0.86,
    ...
    "SlowAPI": _diag(
        "API latency spike — requests timing out at edge", 0.75,
    ...
    "ErrorBudgetBurn": _diag(
        "SLO error budget burning faster than threshold", 0.88,
    ...
    "P0SiteOutage": _diag(
        "P0 multi-service outage — shared dependency failure", 0.92,
    ...
    "DownstreamTimeout": _diag(
        "Downstream timeouts — upstream callers retrying and stacking", 0.79,
...
_ACTIONS_MAP: dict[str, list[dict[str, Any]]] = {
    "DatabaseConnectionExhaustion": [
        _action("scale_connection_pool", "database", {"max_connections": 200},
                "Increase pool capacity", "L1", "Reduce pool to original size"),
        _action("kill_idle_transactions", "database", {"max_idle_seconds": 30},
                "Terminate idle-in-transaction sessions", "L2"),
    ],
    "High5xxAfterDeploy": [
        _action("rollback_deployment", "checkout", {"revision": "previous"},
                "Rollback to last known good revision", "L3"),
        _action("create_ticket", "dev-team", {"priority": "P1"},
                "Investigate regression root cause", "L1"),
    ],
    "RedisCacheAvalanche": [
        _action("scale_cache", "redis", {"replica_count": 3},
                "Add read replicas", "L2"),
        _action("enable_circuit_breaker", "checkout", {"timeout_ms": 200},
                "Prevent DB overload from cache fallback", "L3"),
    ],
    "PodRestartLoop": [
        _action("increase_memory_limit", "checkout", {"memory_limit": "2Gi"},
                "Increase container memory limit", "L2", "Reduce to original limit"),
        _action("rollback_deployment", "checkout", {"revision": "previous"},
                "Revert to known stable version", "L3"),
    ],
    "CPUThrottling": [
        _action("scale_deployment", "checkout", {"cpu_limit": "1000m"},
                "Raise CPU limit", "L2", "Revert to original CPU limit"),
        _action("create_ticket", "platform-team", {"priority": "P2"}, "Right-size requests", "L1"),
    ],
    "MemoryLeak": [
        _action("scale_deployment", "checkout", {"memory_limit": "4Gi"},
                "Bump memory limit to buy time", "L2", "Revert to original limit"),
    ...
    "CertificateExpiry": [
        _action("renew_certificate", "tls", {"domain": "*.example.com"},
                "Trigger automated renewal", "L1"),
        _action("create_ticket", "security-team", {"priority": "P0"}, "Manual renewal", "L1"),
    ],
    "DNSFailure": [
        _action("switch_dns_resolver", "checkout", {"nameserver": "8.8.8.8"},
                "Switch to backup resolver", "L3", "Restore primary resolver"),
        _action("create_ticket", "netops", {"priority": "P1"}, "Investigate DNS", "L1"),
    ],
    "MessageQueueLag": [
        _action("scale_consumers", "checkout", {"replicas": 4},
                "Increase consumer count", "L2", "Scale back to normal"),
        _action("create_ticket", "data-team", {"priority": "P2"}, "Check partitioning", "L1"),
    ],
    "RateLimitTriggered": [
        _action("raise_rate_limit", "checkout", {"max_qps": 2000},
                "Increase rate limit ceiling", "L3", "Restore original limit"),
        _action("create_ticket", "platform-team", {"priority": "P2"}, "Review policy", "L1"),
    ],
    "SlowAPI": [
        _action("enable_caching", "checkout", {"ttl_seconds": 60},
                "Enable response caching", "L2", "Disable cache"),
        _action("create_ticket", "dev-team", {"priority": "P1"}, "Profile slow endpoints", "L1"),
    ],
    "ErrorBudgetBurn": [
        _action("enable_rate_limit", "checkout", {"max_qps": 500},
                "Throttle traffic to protect SLO", "L3", "Remove rate limit"),
        _action("create_ticket", "sre-team", {"priority": "P1"}, "Root-cause budget burn", "L1"),
    ],
    "P0SiteOutage": [
        _action("failover", "primary", {"target": "secondary"},
                "Failover to secondary region", "L3", "Failback to primary"),
        _action(
            "create_ticket", "incident-commander", {"priority": "P0"},
            "Declare major incident", "L1",
        ),
    ],
    "DownstreamTimeout": [
        _action("enable_rate_limit", "checkout", {"max_qps": 400},
                "Throttle to stop retry storm", "L3", "Remove limit when downstream recovers"),
