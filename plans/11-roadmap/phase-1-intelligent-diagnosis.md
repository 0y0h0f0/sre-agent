...
> **Phase 1 完成记录**：Provider factory、推理深度分层、LLM 调用元数据、结构化 rationale、evidence_id 引用和 FakeLLM 确定性测试路径已纳入当前实现。真实 provider smoke 属于手动 demo/eval 配置项，不作为 CI 稳定门禁。

## 1.1 LLM 分层策略

> **实现状态（已完成）**：Provider 抽象层已实现，`_build_deps()` 不再硬编码 `FakeLLM()`。
...
>
> 真实环境验证项：`vllm` + Qwen2.5-7B-AWQ 端到端 smoke、云端 API 单次成本 < $0.05、深度推理实测延迟。这些作为手动 demo/eval 记录维护，不影响默认 FakeLLM 测试路径。

**核心原则**：开发测试默认用 Qwen2.5-7B-Instruct-AWQ 本地验证流程；Qwen3-8B-AWQ 作为可选高风险验证项。生产环境按部署条件选择云端 API 或高参数本地模型。LLM Provider 抽象层已完成配置化切换；跨供应商差异仍通过 adapter 配置和手动 demo/eval 验证。
...
- Phase 1 当前完成口径：provider factory、FakeLLM 稳定路径、真实 provider 配置入口、4 类 smoke eval 数据集、结构化 rationale 和 evidence_id 引用均已纳入仓库；真实 provider 端到端 smoke 作为手动 demo/eval 执行。
...
> **实现状态（已完成）**：每节点推理深度由配置驱动，可整体关闭以加速本地迭代。
...
>
> 真实环境验证：深度推理节点的实测延迟（本地 GPU smoke / 云端 API ≤10s）需真实模型，作为手动 demo/eval 记录维护。
