...
> **实现状态（已完成，2.1-2.4）**：工具层改为「可插拔后端 + factory」结构，所有后端默认 `fixture`，保持测试确定性与本地离线；切到真实只读后端只改配置，不改调用方。
...
>
> **边界遵从**：K8s/DB 均为只读默认 fixture；无真实生产写操作，L4 仍直接拒绝，L3 仍二次确认。Jaeger/Tempo/GitHub/Argo CD/K8s/PG 的端到端 live smoke 属于环境级手动验证，不改变默认 fixture/mock 路径。
